            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              {items.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center space-y-4">
                  <div className="w-16 h-16 border border-dashed border-zinc-800 flex items-center justify-center">
                    <span className="text-xs font-mono text-zinc-600">EMPTY</span>
                  </div>
                  <div className="space-y-1">
                  ...
                  <button
                    onClick={() => setIsOpen(false)}
                    className="px-6 py-2.5 bg-white text-black text-xs font-mono font-bold tracking-widest hover:bg-zinc-200 transition-colors uppercase mt-2"
                  >
                    Browse Collection
                  </button>
                </div>
                ...
                items.map((item) => (
                  <div
                    key={item.variantId}
                    className="flex gap-4 p-4 border border-border-blueprint bg-charcoal-900/50 hover:bg-charcoal-900 transition-all group"
                  >
                    {/* Image */}
